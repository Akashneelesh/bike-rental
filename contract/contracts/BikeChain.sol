...
contract BikeChain {
    ...
    function addRenter(address payable walletAddress,string memory firstName,string memory lastName,bool canRent,bool active,uint balance,uint due,uint start,uint end) public {
        renters[walletAddress] = Renter(walletAddress,firstName,lastName,canRent,active,balance,due,start
        ,end);
    }
    ...
    function makePayment(address walletAddress) payable public {
        require(renters[walletAddress].due > 0, "You do not have a due this time");
        require(renters[walletAddress].balance > msg.value, "You do not have enough balance");
        renters[walletAddress].balance -= msg.value;
